  ...
  const handleSubcategoryChange = (
    ...
    if (isChecked) {
      setSelectedSubcats((prev) => {
        ...
        const selectedCount = newSubs.filter((id) => subcategoryIds.includes(id)).length
      ...
      // When unchecking a subcategory
      setSelectedSubcats((prev) => {
        // Remove the unchecked subcategory
        const newSubs = prev.filter((id) => id !== subcategory.id)

        // If parent was selected (meaning all subcategories were conceptually selected)
        if (selectedCats.includes(parentCategory.id)) {
          // Add all remaining subcategories to the selection
          const remainingSubIds =
            parentCategory.subcategories?.map((s) => s.id).filter((id) => id !== subcategory.id) ?? []

          return Array.from(new Set([...newSubs, ...remainingSubIds]))
        }

        // If parent wasn't selected, just remove the unchecked subcategory
        return newSubs
      })
    }
  }
                ...
                <div className="flex items-center justify-between py-1">
                  <div className="flex items-center gap-2 font-thin">
                    ...
                    <label
                      ...
                      className="cursor-pointer text-sm text-[#213E5A] font-normal"
                    >
                      {category.name}
                    </label>
                  </div>
