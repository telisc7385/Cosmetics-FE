...
export default function ShopPageClient({ categories }: Props) {
  ...
  const [tagNameToIdMap, setTagNameToIdMap] = useState<Map<string, number>>(new Map())
  ...
  useEffect(() => {
    ...
    const qTags = selectedTagIds.length > 0 ? `tags=${selectedTagIds.join(",")}` : ""
  ...
  }, [selectedCats, selectedTags, sortOrder, min, max, currentPage, base, tagNameToIdMap]) // Add tagNameToIdMap to dependencies
