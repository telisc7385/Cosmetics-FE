...
const UserCheckout = () => {
    ...
    async function fetchAddresses() {
      ...
      try {
        ...
        if (fetchedAddresses.length > 0) {
          ...
          const billingId =
            sb && fetchedAddresses.some((a) => a.id === sb)
              ? sb
              : defaultAddr.id;
          const shippingId = same
            ? billingId
            : ss && fetchedAddresses.some((a) => a.id === ss)
            ? ss
            : null;
  ...
  useEffect(() => {
    selectedBillingId
      ...
      : localStorage.removeItem("selectedBillingId");
    selectedShippingId
      ? localStorage.setItem("selectedShippingId", selectedShippingId)
      : localStorage.removeItem("selectedShippingId");
  ...
  const handlePlaceOrder = async () => {
    ...
    if (cartLoading || addressIsLoading)
      return toast.error("Please wait until everything loads.");
    ...
    <div className="flex flex-col md:flex-row gap-6 p-6 bg-gray-100 min-h-screen max-w-7xl mx-auto">
      ...
      <div className="w-full md:w-1/2 bg-white p-6 rounded shadow">
        ...
        {addressIsLoading ? (
          ...
          addressHasError && <p className="text-red-600">{addressHasError}</p>
        )}
        {renderAddressList(
          address,
          selectedBillingId,
        ...
        {!sameAsBilling && (
          <div className="mt-6">
            ...
            {addressIsLoading ? (
              ...
              addressHasError && (
                <p className="text-red-600">{addressHasError}</p>
              )
            )}
            {renderAddressList(
              address,
